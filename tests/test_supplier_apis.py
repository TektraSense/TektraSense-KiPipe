...
def test_call_digikey_api_request_exception(mocker, mock_env_vars):
    """Tests DigiKey API call failure due to a network error."""
    # 1. Arrange
...
def test_call_mouser_api_success(mocker, mock_env_vars):
    """Tests a successful Mouser API call where the part number matches."""
    # 1. Arrange
    mock_resp = MagicMock()
...
def test_call_mouser_api_no_parts_found(mocker, mock_env_vars):
    """Tests Mouser API when the search returns no parts."""
    # 1. Arrange
    mock_resp = MagicMock()
