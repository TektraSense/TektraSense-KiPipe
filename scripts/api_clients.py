...
CATEGORY_RECIPES = [
    {
        ...
        "value_generator": lambda find, path: ", ".join(filter(None, [
    ...
    },
    {
        ...
        "description_prefix": lambda path: path[-1],
        "description_params": lambda path: ["Inductance", "Tolerance", "Current Rating (DC)", "DC Resistance (DCR)", "Package / Case", "Features", "Ratings"],
        "value_generator": lambda find, path: ", ".join(filter(None, [
            find('Inductance'),
            find('Tolerance'),
            find('Current Rating (DC)')
        ]))
    },
    {
